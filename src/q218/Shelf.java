...
//   		return false;
	 ...
	 *         checks passed. Book book1, book2; book1 = new Book(44, "shel
	 *         silverstein" ,"the Giving tRee",532); book2 = new Book(434, "Dan
	 *         Brown" ,"the da Vinci coDe",1532); Shelf.addBook(book1); => true
	 ...
	 * @return Book object Assume we have Book id=100, title=Start With Why, Author:
	 *         Simon Sinek Shelf.findBookByPartialTitle("start"); => returns that
	 *         Book object Shelf.findBookByPartialTitle("why"); => returns that Book
	 *         object Shelf.findBookByPartialTitle("Wooden Spoon); returns null
	 */
	...
	/*
	 * NE KADAR SACMA YAPMISIM ASAGIDA YAWS!!!! BU DA UZUN YOLDAN YAPIMI YUKARIDA
	 * KISALTTIM title = title.toLowerCase(); String objTitle =
	 * theBooks.get(i).getTitle().toLowerCase();
	 * 
	 * if (objTitle.contains(title) ) { return theBooks.get(i); } }
	 */
	/**
...
//		theBooks.remove(theBooks.get(theBooks.indexOf(id)));
		for (int i = 0; i < theBooks.size(); i++) {
			if (theBooks.get(i).getId() == id) {
