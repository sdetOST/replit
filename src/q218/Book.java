...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
    	ArrayList <String> list = new ArrayList <> (Arrays.asList("a" , "an", "the", "and", "but", "for", "nor", "or", 
    														"so", "yet","at", "by", "in", "into", "near", "of", "on", "to", "than", "via"));
    ...
    public void setAuthor(String author) {
    	...
    	if(author.equals("")) {
    		...
    		String[] str = author.split(" ");
     		...
     		this.author = words.trim();							//    		words = words.substring(0, words.length()-1);
    	}
     }
    /**
    ...
    public void setTitle(String title) {
    	...
    	if (!title.contains(" ")) this.title = capitalize(this.title);
    	else if (title.equals("")) this.title = "";
    	else {
    		String[] str = title.toLowerCase().split(" ");
    		String words = capitalize(str[0]) + " ";
    		for (int i = 1; i < str.length; i++) {
     			if(isTitleWord(str[i])) str[i] = capitalize(str[i]);
     			
     			words += str[i]+" ";
     		}
     		this.title = words.trim();								// this.title =	words.substring(0, words.length()-1);
    	}
    }
